...
//base url
String url =
    'http://orbit-ride.com/'; //add '/' at the end of the url as 'https://url.com/'
String mapkey =
    (platform == TargetPlatform.android) ? 'AIzaSyBM4V2aF4-sWyACoH7IH9KPEb58Ij9i4us' : 'AIzaSyBM4V2aF4-sWyACoH7IH9KPEb58Ij9i4us';
...
registerOwner() async {
  ...
  try {
    ...
    // print('t type ' + transportType.toString());
    var request = await response.send();
    var respon = await http.Response.fromStream(request);
...
dynamic aproximateDistance;
dynamic aproximateDistance1;
dynamic aproximateMins;
        ...
        if (userDetails['onTripRequest'] != null) {
          ...
          if (tripStops.isNotEmpty) {
            for (var i = 0; i < tripStops.length; i++) {
              // if (tripStops[i]['completed_at'] == null) {
              addressList.add(AddressList(
                  id: (i + 2).toString(),
                  type: 'drop',
                  // pickup: false,
                  ...
                  completedat: (tripStops[i]['completed_at'] != null)
                      ? tripStops[i]['completed_at']
                      : null));
              // }
            }
          ...
          if (aproximateDistance > 0 && aproximateDistance <= 1) {
            aproximateMins = '2 mins';
          } else if (aproximateDistance > 1 && aproximateDistance <= 3) {
            aproximateMins = '5 mins';
          } else if (aproximateDistance > 3 && aproximateDistance <= 5) {
            aproximateMins = '8 mins';
          } else if (aproximateDistance > 5 && aproximateDistance <= 7) {
            aproximateMins = '11 mins';
          } else if (aproximateDistance > 7 && aproximateDistance <= 10) {
            aproximateMins = '14 mins';
          } else if (aproximateDistance > 10) {
            aproximateMins = '15 mins';
          }
        } else {
          duration = 0;
...
stopComplete(id) async {
  dynamic result;
  try {
    var response =
        await http.post(Uri.parse('${url}api/v1/request/stop-complete'),
            headers: {
              'Authorization': 'Bearer ${bearerToken[0].token}',
              'Content-Type': 'application/json'
            },
...
driverTodayEarning() async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      // printWrapped('daily ' + response.body);
    } else if (response.statusCode == 401) {
      result = 'logout';
    } else {
      debugPrint(response.body);
...
driverWeeklyEarning() async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      // printWrapped('weekly ' + response.body);
    } else if (response.statusCode == 401) {
      result = 'logout';
    } else {
      debugPrint(response.body);
...
driverEarningReport(fromdate, todate) async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      // printWrapped('report ' + response.body);
      result = 'success';
    } else if (response.statusCode == 401) {
      result = 'logout';
    } else {
...
geolocs.LocationSettings locationSettings = (platform == TargetPlatform.android)
    ? geolocs.AndroidSettings(
        ...
        foregroundNotificationConfig:
          ...
          notificationText:
              "Orbit Ride will continue to receive your location in background",
      ...
      list.forEach((key, value) {
        if (value['drivers'] != null &&
          ...
          if (value['drivers']['driver_${userDetails["id"]}'] != null) {
            ...
            userDetails['vehicle_types'].contains(value['vehicle_type'])) {
          // Check if request_no is in the list
          if (requestDriverList.isEmpty) {
            RemoteNotification noti = RemoteNotification(
                title: languages[choosenLanguage]['text_new_trip_req'],
                body: languages[choosenLanguage]['text_new_trip_req_bid']);
            showRideNotification(noti);
            requestDriverList.add(value);
          }

          rideList.add(value);
          String searchRequestNo = value['request_no'];

          bool containsRequestNo = requestDriverList
              .any((request) => request['request_no'] == searchRequestNo);
          if (containsRequestNo) {
          } else {
            RemoteNotification noti = RemoteNotification(
                title: languages[choosenLanguage]['text_new_trip_req'],
                body: languages[choosenLanguage]['text_new_trip_req_bid']);
...
getLandingImages() async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      loginImages.sort((a, b) => a['order'].compareTo(b["order"]));
      phcode =
          (countries.where((element) => element['default'] == true).isNotEmpty)
              ? countries.indexWhere((element) => element['default'] == true)
              : 0;
